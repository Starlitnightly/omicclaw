...
OmicClaw offers two installation methods.
...
```bash
omicclaw --port 8080          # Specify a port
omicclaw --no-debug           # Disable debug mode (recommended for production)
omicclaw --remote             # Remote mode (use with SSH tunnel)
```
...
To run OmicClaw on a remote server and access it from a local browser via an SSH tunnel:
...
- [OmicClaw package source](https://github.com/Starlitnightly/omicclaw)
